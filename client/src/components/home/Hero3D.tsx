  ...
  return (
    ...
    >
      <div className="relative overflow-hidden rounded-3xl border border-white/[0.09] bg-gradient-to-b from-white/[0.07] via-transparent to-transparent p-5 shadow-[0_24px_80px_-12px_rgba(0,0,0,0.45)] ring-1 ring-inset ring-white/[0.04] sm:p-6 md:p-8">
        <div className="pointer-events-none absolute -right-20 -top-24 h-64 w-64 rounded-full bg-orange-500/15 blur-3xl" aria-hidden />
        <div className="pointer-events-none absolute -bottom-16 -left-16 h-56 w-56 rounded-full bg-sky-500/10 blur-3xl" aria-hidden />
        ...
        <div className="relative grid min-h-[240px] grid-cols-12 gap-3 sm:min-h-[280px] md:min-h-[300px]">
          <div className="col-span-12 flex flex-col rounded-2xl border border-white/[0.08] bg-black/25 p-4 backdrop-blur-md sm:col-span-7">
            ...
            <div className="mt-4 grid grid-cols-4 gap-2">
              {[40, 56, 32, 64].map((h, i) => (
                <div key={i} className="flex h-24 flex-col justify-end rounded-lg border border-white/[0.06] bg-white/[0.04] px-1.5 pb-2 pt-3 sm:h-28">
                  <div className="mx-auto w-full max-w-[2rem] rounded-sm bg-gradient-to-t from-orange-500/45 to-orange-400/15" style={{ height: `${h}px` }} />
                </div>
              ))}
            </div>
...
function FloatingProductLink({
  ...
  const ring = accent === "orange" ? "hover:border-orange-500/40 hover:shadow-orange-500/10" : "hover:border-sky-500/40 hover:shadow-sky-500/10";
  const iconBg = accent === "orange" ? "bg-orange-500/15 text-orange-400" : "bg-sky-500/15 text-sky-400";
